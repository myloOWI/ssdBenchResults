...
def process_results_directory(dir_path: Path) -> dict[str, tuple[float, float]]:
    ...
    Returns a mapping from json stem names to ``(bandwidth_MiB_s, iops)``
    values for aggregation across runs.
    """
...
def aggregate_results(all_runs: dict[str, dict[str, tuple[float, float]]]) -> None:
    ...
    with open(output_dir / "summary.csv", "w", newline="") as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(["file", "run", "bandwidth_MiB_s", "iops"])
        for run, mapping in all_runs.items():
            for file, (bw, iops) in mapping.items():
                writer.writerow([file, run, bw, iops])
    ...
    bandwidth_matrix = [[all_runs[run].get(f, (0, 0))[0] for f in files] for run in runs]
    iops_matrix = [[all_runs[run].get(f, (0, 0))[1] for f in files] for run in runs]
...
def main() -> None:
    all_runs: dict[str, dict[str, tuple[float, float]]] = {}
